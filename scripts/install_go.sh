...
function main() {
  # FIPS Go buildpack uses cflinuxfs5 stack
  if [[ "${CF_STACK:-cflinuxfs5}" != "cflinuxfs5" ]]; then
    echo "       **ERROR** Unsupported stack: ${CF_STACK:-}"
    echo "                 This FIPS buildpack only supports cflinuxfs5"
  ...
  if [[ ! -f "${dir}/bin/go" ]]; then
    ...
    echo "-----> Download go ${version} (for buildpack compilation)"
    curl "${url}" \
      --silent \
      --location \
      --retry 15 \
