...
Packages the FIPS Go buildpack into a .zip file.
OPTIONS
  ...
  --stack <stack>                    target stack (default: cflinuxfs5)
  --output <path>                    output path for the buildpack zip
USAGE
}
...
function package::buildpack() {
  ...
  echo "Building FIPS Go buildpack (version: ${version}, stack: ${stack}, cached: ${cached}, output: ${output})"
