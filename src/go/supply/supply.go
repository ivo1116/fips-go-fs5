...
func Run(gs *Supplier) error {
	gs.Log.BeginStep("FIPS Go Buildpack")
	gs.Log.Info("This buildpack uses golang-fips/go with system OpenSSL for FIPS compliance")
	...
	if err := gs.WriteFIPSProfileD(); err != nil {
		gs.Log.Error("Error writing FIPS configuration to profile.d: %s", err.Error())
		return err
	}
...
// WriteFIPSProfileD writes the FIPS configuration script to profile.d
func (gs *Supplier) WriteFIPSProfileD() error {
	gs.Log.BeginStep("Configuring FIPS environment")
...
func (gs *Supplier) InstallGo() error {
	...
	gs.Log.BeginStep("Installing FIPS Go %s", gs.GoVersion)
	...
	// Set FIPS-specific environment variables
	if err := gs.Stager.WriteEnvFile("GOLANG_FIPS", "1"); err != nil {
		return err
	}
